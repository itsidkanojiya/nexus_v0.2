...
const questionSlice = createSlice({
  ...
  reducers: {
    ...
    addQuestion: (state, action) => {
      const exists = state.selectedQuestions?.some((question) => question.id === action.payload.id);
      if (!exists) {
    ...
    setQuestionsType: (state, action) => {
      ...
      if (action?.payload) {
        state.filteredQuestion = state?.questionsList?.filter((question) => question?.type === action.payload);
      } else {
    ...
    setSearch: (state, action) => {
      ...
      state.filteredQuestion = state?.questionsList?.filter((question) => question?.question?.includes(action?.payload));
    },
...
export const { setQuestionsList, setFilteredQuestion, addQuestion, removeQuestion, setMarks, setQuestionsType, setSearch, resetQuestions } = questionSlice.actions;
export default questionSlice.reducer;

// Selector to check if a question is in selectedQuestions
export const selectIsQuestionSelected = (state, questionId) => state.selectedQuestions?.some((question) => question.id === questionId);

// Selector to get selected questions with their types and marks
