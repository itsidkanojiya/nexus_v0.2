...
const AssignmentList = ({ goNext, goLast }) => {
    const { token, user } = useAuth();
    const setPaper = usePaperStore((state) => state.setPaper);

    const { data, isLoading, isError, error } = useQuery({
        queryKey: ["getAssignmentHistory"],
        queryFn: () => getData("get-assignment-history", token),
        enabled: !!token,
    });
                                    ...
                                    <button
                                        onClick={() => {
                                            ...
                                            if (allQuestions && paper) {
                                                ...
                                                dispatch(
                                                    setFilteredQuestion(newList)
                                                );
                                                setQuestionsForTypes(
                                                    paper?.questions
                                                );
                                            }
                                            const marks = {
                                                mcq: paper.questions.mcq.marks,
                                                blanks: paper.questions.blanks
                                                    .marks,
                                                true_false:
                                                    paper.questions.true_false
                                                        .marks,
                                                onetwo: paper.questions.onetwo
                                                    .marks,
                                                short: paper.questions.short
                                                    .marks,
                                                long: paper.questions.long
                                                    .marks,
                                            };
