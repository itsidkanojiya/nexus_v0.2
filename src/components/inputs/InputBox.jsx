export default function InputBox({ register = () => {}, errors = {}, label = "Add Label", topLabel = true, isRequired = true, name, placeholder, className = "", type = "text", handleRightClick = () => {}, handleLeftClick = () => {}, rightIcon = null, leftIcon = null }) {
  return (
    <div>
      <div className="relative">
        ...
        <div className="relative">
          <input {...register(name, { required: isRequired ? `${label} is required.` : false })} autoComplete="false" type={type} placeholder={placeholder} className={`w-full p-2.5 border-2 rounded-md focus:outline-none ${rightIcon ? "pe-8" : leftIcon ? "ps-8" : ""}  ${className} ${errors?.[name] ? "border-red-600" : ""}`} />
          {topLabel && (
